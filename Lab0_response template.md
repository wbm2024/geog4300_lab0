...
In the tornado dataset, the “STATE” column is Nominal data because it is
just the name of the state the tornado occurred in. It can’t be ranked,
put in order, or multiplied/divided. Also in the tornado dataset, the
“DEATHS_DIRECT” dataset is ratio data, because the amount has a definite
0, and a value can be multiplies (50 deaths is double 25 deaths). The
TOR_F_SCALE, which is the tornado rating, would be ordinal data since it
rated 0 through 5, but 4 isn’t twice as strong as 2. Since it can be
ranked though, I think that data could be better served as a factor. I
think this because it has finite levels, ranging from F (or EF) 0, to
F/EF-5.
...
I chose the F_MOBILE variable, which is the counties with the 90th
percentile of percentages of mobile homes within a county. I chose this
variable because mobile homes are able to be completely destroyed with
even a lower rated tornadoes, which putting them at risk, even if they
take shelter in their home. Counties with a large percentage of these
mobile homes are at a greater potential for higher fatalities.
...
```
    ...
    Joining with `by = join_by(cty_fips)`
...
Inner joins take the data within the rows in dataset A and join them to
the data in rows from dataset B. Any matching data from dataset A will
be kept, and anything that does not have a matching value in dataset B
will be dropped from the join and not transferred. In a full join,
everything from both dataset A and B, even if there is not matching
data, is joined, with the unmatched data from dataset B still attached
to the joined dataset.
...
Walker County, Alabama had the most with 11 total F/EF-3+ Tornadoes. De
Soto Parish, Louisiana has the 2nd most with 9 total F/EF-3+ tornadoes.
Copiah County, Mississippi had the 3rd most tornadoes with 7 F/EF-3+
rated tornadoes. Walker County, Alabama does not have any major cities,
but Jasper, the largest town in the county, hasover 14,500 people.
Walker County is also close to Birmingham, AL. De Soto Parish, LA also
does not have a major city, with Mansfield, LA being the most populated
at just under 5,000 people. It is, however, close to Shreveport, LA.
Lastly, Copiah County, Mississippi also does not have a major city
within it, with their largest towns both under 5,000 people each. It
does follow the trend of the previous 2 counties in the sense that it is
located close to a major city, in this case Jackson, MS.
...
Paper: Assessing United States County-Level Exposure for Research on
Tropical Cyclones and Human Health

Citation: Anderson, G. B., Ferreri, J., Al-Hamdan, M., Crosson, W.,
Schumacher, A., Guikema, S., Quiring, S., Eddelbuettel, D., Yan, M., &
Peng, R. D. (2020). Assessing United States County-Level Exposure for
Research on Tropical Cyclones and Human Health. Environmental health
perspectives, 128(10), 107009. https://doi.org/10.1289/EHP6976

Response to challenge question: The study aimed to answer questions
regarding how hazards associated with tropical cyclones, those being
wind, rain, flood, and tornadoes, impacted the United States exposure at
a county level. The data used included the NOAA Storm Events Database
and HURDAT for the tropical cyclone track, as well as R for the coding
software. First the study defined that any tropical cyclone track within
250 km from a U.S. county would be considered, then they defined the
distance away from the tropical cyclone center and timing range around
the landfall for each hazard, which generally ended up being 500 km away
and within 2 days of the tropical cyclone track’s closest approach. Rain
only varied from the timing methodology, as it was 2 days before and 1
day after the storms closest approach to properly account for rainfall.
Wind also varied because HURDAT defines the area around the storm’s
center that received high winds of 34 knots or greater. Some notable
results were that rain was the hazard that had the most exposed counties
(both per year IQR and in single storm high count), while tornadoes had
the least exposed counties. Another notable result is that hazards range
from local issue to widespread problem. They show that tornadoes are
mostly localized to a small number of counties, mostly in Florida, while
rainfall impacts almost every county from Texas to Maine.
...
I used Google and Bing to find out what errors meant, and if I was
missing any key characters like pipes, which I forget frequently. These
tools guided me so that I was not scrolling my code for hours trying to
find an error that was minimal and just part of the learning curve of
using a new software. Furthermore, error outputs can be confusing, so
explanations on what they meant in simple terms helped me solve my
...
In general, it was pretty easy. I did forget the pipes enough to cause
slowdowns but I think as I learn this software more I will get better at
it. I learned about the basics of data wrangling in R, and how it can
simplify efficiently handle large datasets.
